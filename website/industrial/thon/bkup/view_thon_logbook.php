...
if ($_GET['action'] == 'map') {
    ...
    $query_t = "SELECT navire, date_c, ST_Y(location), ST_X(location) FROM thon.logbook";
    ...
    ?>
    <form method="post" action="<?php echo $self;?>?source=thon&table=logbook&action=show" enctype="multipart/form-data">
    <fieldset>
    ...
    <tr>
    <td><input type="text" size="20" name="f_s_navire" value="<?php echo $_SESSION['filter']['f_s_navire']?>"/></td>
    <td>
    <select name="f_id_species">
        <option value="id_species" selected="selected">Tous</option>
        <?php
        $result = pg_query("SELECT DISTINCT fishery.species.id, fishery.species.francaise, fishery.species.family, fishery.species.genus, fishery.species.species FROM fishery.species  JOIN thon.logbook ON fishery.species.id = thon.logbook.id_species  ORDER BY  fishery.species.family, fishery.species.genus, fishery.species.species");
    ...
    <td><b>Port et date depart / arrive</b></td>    
    <td><b>Numbre jour maree</b></td> 
    <td><b>Date heure lanc&eacute;</b></td>    
    <td><b>Numbre lanc&eacute;</b></td>
    <td><b>EEZ</b></td>
    <td><b>Temperature Eau / Vitesse vent / Direction vent / Vitesse courant</b></td>
    <td><b>Coup null</b></td>
    ...
    if ($_SESSION['filter']['f_s_navire'] != "" OR $_SESSION['filter']['f_id_species'] != "" OR $_SESSION['filter']['f_s_date'] != "" ) {
        ...
        if ($_SESSION['filter']['f_s_navire'] != "") { 
            $query = "SELECT count(logbook.id) FROM thon.logbook "
                . "WHERE id_species=".$_SESSION['filter']['f_id_species']." ";
            
            $pnum = pg_fetch_row(pg_query($query))[0];
            
            $query = "SELECT logbook.id, logbook.username, logbook.datetime, navire, country, port_d, port_a, date_d, date_a, ndays, date_c, heure_c, lance, eez, water_temp, wind_speed, wind_dir, cur_speed, success, banclibre, balise_id, rejete, fishery.species.id, fishery.species.francaise, fishery.species.family, fishery.species.genus, fishery.species.species, taille, poids, comment, st_x(location), st_y(location), "
            . " coalesce(similarity(thon.logbook.navire, '".$_SESSION['filter']['f_s_navire']."'),0) AS score"
            . " FROM thon.logbook "
            . "LEFT JOIN fishery.species ON thon.logbook.id_species = fishery.species.id "
            . "WHERE id_species=".$_SESSION['filter']['f_id_species']." "
    ...
    while ($results = pg_fetch_row($r_query)) {
        ...
        print "<td>"
        . "<a href=\"./view_thon_logbook.php?source=$source&table=$table&action=edit&id=$results[0]\">Modifier</a><br/>"
        . "<a href=\"./view_thon_logbook.php?source=$source&table=$table&action=delete&id=$results[0]\" onclick=\"return confirm('Voulez-vous vraiment supprimer l\'enregistrement?')\">Effacer</a>"
        . "</td>";
        print "<td>$results[1]<br/>$results[2]</td><td>$results[3]</td><td>$results[4]</td><td>$results[5] $results[6] / $results[7] $results[8]</td><td>$results[9]</td><td>$results[10] $results[11]</td>"
        . "<td>$results[12]</td><td>$results[13]</td><td>$results[14]/$results[15]/$results[16]/$results[17]</td><td>$results[18]</td><td>$results[19]</td><td>$results[20]</td>"
        . "<td>$results[21]</td><td>".formatSpecies($results[23],$results[24],$results[25],$results[26])."</td>"
        . "<td>$results[27]</td><td>$results[28]</td><td>$results[29]</td><td><a href=\"view_point.php?X=$results[30]&Y=$results[31]\">".round($results[30],3)."E ".round($results[31],3)."N</a></tr>";
    }
    print "</tr>";
    print "</table>";
    pages($start,$step,$pnum,'./view_thon_logbook.php?source=thon&table=logbook&action=show&f_s_navire='.$_SESSION['filter']['f_s_navire'].'&f_id_species='.$_SESSION['filter']['f_id_species']);
    ...
    $q_id = "SELECT *, fishery.species.id, fishery.species.FAO, fishery.species.family, fishery.species.genus, fishery.species.species, st_x(location), st_y(location) FROM thon.logbook "
        . "LEFT JOIN fishery.species ON fishery.species.id = thon.logbook.id_species "
        . "WHERE logbook.id = '$id'";
    ...
    $id = $_GET['id'];
    $query = "DELETE FROM thon.logbook WHERE id = '$id'";
    
    if(!pg_query($query)) {
        msg_queryerror();
//        print $query;
    } else {
        header("Location: ".$_SESSION['http_host']."/industrial/view_thon_logbook.php?source=$source&table=logbook&action=show");
    }
    $controllo = 1;
}
    ...
    for ($i = 0; $i < 5; $i++) {
        ...
        if ($id_species[$i] != '' and $taille[$i] != '' and $poids[$i] != '') {
            ...
            if ($_POST['new_old']) {
                ...
                $query = "INSERT INTO thon.logbook "
