...
print "<h3>Maree details</h3>";
        ...
        $query = "SELECT maree.id, maree.username, maree.datetime::date, navire, country, year, port_d, date_d, port_a, date_a"
        . " FROM thon.maree "
...
<?php
        $query = "SELECT lance.id, lance.username, lance.datetime, id_maree, navire, maree.year, date_c, heure_c, eez, success, banclibre, balise_id, water_temp, wind_speed, wind_dir, cur_speed, comment, st_x(location), st_y(location)"
        . " FROM thon.lance "
...
if (pg_num_rows($r_query) > 0) {
    ...
    while ($results = pg_fetch_row($r_query)) {
        ...
        print "<td>$results[1]<br/>$results[2]</td><td nowrap>$results[4]<br/>$results[5]</td>"
                . "<td nowrap><a href=\"./view_lance.php?id=$results[0]&source=thon&table=lance&action=show\">$results[6]<br/>$results[7]</a></td><td>$results[8]</td><td>$results[9]</td><td>$results[10]</td>"
