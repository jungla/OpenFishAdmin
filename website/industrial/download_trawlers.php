...
if ($_GET['action'] == 'download') {
    ...
    $filename = 'Observateurs_Chalutier_'.$table.'.csv';

    if ($table == 'route') {

        $q_id = "SELECT route.id, route.datetime, route.username, navire, maree, t_fleet.fleet, date, lance, h_d, h_f, depth_d, depth_f, speed, reject, sample, comment,  st_x(location_d), st_y(location_d), st_x(location_f), st_y(location_f)  "
        . " FROM trawlers.route "
        ...
        $header = ['id', 'datetime', 'username', 'navire', 'maree', 'fleet', 'date', 'lance', 'time debut', 'heure fin', 'profondeur debut', 'profondeur fin', 'vitesse', 'poids reject', 'poids echantillon', 'remarque', 'GPS_x debut', 'GPS_y debut', 'GPS_x fin', 'GPS_y fin'];
