...
if ($_GET['action'] == 'map') {
    ...
    $query_t = "SELECT navire, date, ST_Y(location), ST_X(location) FROM trawlers.captures_requin";
    ...
    <select name="f_id_species" class="chosen-select" >
        ...
        <?php
        $result = pg_query("SELECT DISTINCT fishery.species.id, fishery.species.FAO, fishery.species.francaise, fishery.species.family, fishery.species.genus, fishery.species.species FROM trawlers.captures_requin LEFT JOIN fishery.species ON captures_requin.id_species = fishery.species.id ORDER BY fishery.species.francaise, fishery.species.family, fishery.species.genus, fishery.species.species");
    ...
    if ($_SESSION['filter']['f_s_maree'] != "" OR $_SESSION['filter']['f_id_species']) {
        ...
        if ($_SESSION['filter']['f_s_maree'] != "") {
            $query = "SELECT count(captures_requin.id) FROM trawlers.captures_requin "
                . "WHERE id_species=".$_SESSION['filter']['f_id_species']." ";

            $pnum = pg_fetch_row(pg_query($query))[0];

            $query = "SELECT captures_requin.id, captures_requin.datetime::date, captures_requin.username, id_route, route_accidentelle.maree, route_accidentelle.date, route_accidentelle.time, fishery.species.id, fishery.species.francaise, fishery.species.family, fishery.species.genus, fishery.species.species, n_ind, t_sex.sex, taille, poids, t1.condition, t2.condition, preleve, camera, photo, remarque, "
            . " coalesce(similarity(trawlers.captures_requin.maree, '".$_SESSION['filter']['f_s_maree']."'),0) AS score"
            ...
            . "LEFT JOIN trawlers.t_condition t2 ON trawlers.captures_requin.t_relache = t2.id "
            . "WHERE id_species=".$_SESSION['filter']['f_id_species']." "
            . "ORDER BY captures_requin.datetime DESC OFFSET $start LIMIT $step";
        }
    } else {
        $query = "SELECT count(captures_requin.id) FROM trawlers.captures_requin";
        $pnum = pg_fetch_row(pg_query($query))[0];
    ...
    while ($results = pg_fetch_row($r_query)) {
        ...
        print "<a href=\"./view_trawlers_captures_requin.php?source=$source&table=$table&action=edit&id=$results[0]\">Modifier</a><br/>"
            . "<a href=\"./view_trawlers_captures_requin.php?source=$source&table=$table&action=delete&id=$results[0]\" onclick=\"return confirm('Voulez-vous vraiment supprimer l\'enregistrement?')\">Effacer</a>";
        }
        print "</td>";
        ...
        . "<td>".formatSpecies($results[8],$results[9],$results[10],$results[11])."</td><td>$results[12]<br/>$results[13]</td><td nowrap>$results[14]</td><td>$results[15]</td><td>$results[16] - $results[17]</td><td>$results[18]</td><td>$results[19]<br/>$results[20]</td><td>$results[21]</td></tr>";
    }
    print "</tr>";
    print "</table>";
    pages($start,$step,$pnum,'./view_trawlers_captures_requin.php?source=trawlers&table=captures_requin&action=show&f_s_maree='.$_SESSION['filter']['f_s_maree'].'&f_id_species='.$_SESSION['filter']['f_id_species']);
    ...
    $q_id = "SELECT captures_requin.id, captures_requin.datetime, captures_requin.username, id_route, maree, date, time, fishery.species.id, fishery.species.francaise, fishery.species.family, fishery.species.genus, fishery.species.species, n_ind, t_sex, taille, poids, t_capture, t_relache, preleve, camera, photo, remarque "
            . "FROM trawlers.captures_requin "
            . "LEFT JOIN fishery.species ON trawlers.captures_requin.id_species = fishery.species.id "
            . "WHERE captures_requin.id = '$id'";
    ...
    <b>Poids</b> (kg)
    <br/>
    <input type="text" size="10" name="poids" value="<?php echo $results[15]; ?>"/>
    <br/>
    <br/>
    ...
    <br/>
    <input type="text" size="10" name="preleve" value="<?php echo $results[18]; ?>"/>
    <br/>
    <br/>
    <b>Camera ID</b>
    <br/>
    <input type="text" size="20" name="camera" value="<?php echo $results[19]; ?>"/>
    <br/>
    <br/>
    <b>Photo ID</b>
    <br/>
    <input type="text" size="20" name="photo" value="<?php echo $results[20]; ?>"/>
    <br/>
    <br/>
    <b>Remarque</b>
    <br/>
    <input type="text" size="30" name="remarque" value="<?php echo $results[21];?>" />
    <br/>
    <br/>
    ...
    $id = $_GET['id'];
    $query = "DELETE FROM trawlers.captures_requin WHERE id = '$id'";

    if(!pg_query($query)) {
        msg_queryerror();
//        print $query;
    } else {
        header("Location: ".$_SESSION['http_host']."/industrial/view_trawlers_captures_requin.php?source=$source&table=captures_requin&action=show");
    }
    $controllo = 1;
...
if ($_POST['submit'] == "Enregistrer") {
    ...
    if ($_POST['new_old']) {
        ...
        $query = "UPDATE trawlers.captures_requin SET "
            . "username = '$username', datetime = now(), "
            . "id_route = '".$id_route."', maree = '".$maree."', "
            . "date = '".$date."', time = '".$time."', id_species = '".$id_species."', "
            . "n_ind = '".$n_ind."', t_sex = '".$t_sex."', taille = '".$taille."', poids = '".$poids."', t_capture = '".$t_capture."', "
