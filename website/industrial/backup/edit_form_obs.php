...
if ($_GET['action'] == 'show') {
    print "<a href=\"./edit.php\">edit</a> > <a href=\"./edit_form.php?source=$source\">".label2name($source)."</a> > <a href=\"./edit_form_art.php?source=$source&table=$table&action=show\">".label2name($table)."</a>";

    // fetch data
    $query = "SELECT * FROM peche_artisanal.captures ORDER BY datetime DESC";
    ...
    while ($results = pg_fetch_row($r_query)) {
        print "<tr><td>"
        . "<a href=\"./edit_form_art.php?source=$source&table=$table&action=edit&id=$results[0]\">Edit</a><br/>"
        . "<a href=\"./edit_form_art.php?source=$source&table=$table&action=delete&id=$results[0]\">Delete</a>"
        . "</td>";
    ...
    print "<a href=\"./edit.php\">edit</a> > <a href=\"./edit_form_art.php?source=$source\">".label2name($source)."</a> > <a href=\"./edit_form_art.php?source=$source&table=$table&action=show\">".label2name($table)."</a>";

    $id = $_GET['id'];

    //find record info by ID
    $q_id = "SELECT * FROM peche_artisanal.captures WHERE id = '$id' ORDER BY datetime DESC";
    #print $q_id;
    $r_id = pg_query($q_id);
    ...
    if ($results[18] == "t") {
        $query_t = "SELECT st_astext(gps_track) FROM peche_artisanal.captures WHERE id = '$id'";    
    ...
    $id = $_GET['id'];
    $query = "DELETE FROM peche_artisanal.captures WHERE id = '$id'";
    
    if(!pg_query($query)) {
        msg_queryerror();
//        print $query;
    } else {
        header("Location: ".$_SESSION['http_host']."/edit_form_art.php?source=$source&table=captures&action=show");
    }
    $controllo = 1;
...
if ($_POST['delete'] == "Delete Track") {
    $query = "SELECT gps_file FROM peche_artisanal.captures WHERE id = '{".$_POST['id']."}'";
    $gps_file = pg_fetch_row(pg_query($query));
    
    $query = "UPDATE peche_artisanal.captures SET "
...
//        print $query;
        ...
        header("Location: ".$_SESSION['http_host']."/edit_form_art.php?source=$source&table=captures&action=edit&id=".$_POST['id']);
    }
...
if ($_POST['submit'] == "Submit") {
    ...
    if ($_FILES['kml_file']['tmp_name'] != "") {
        $query = "UPDATE peche_artisanal.captures SET "
                ...
                . "t_species = '".$_POST['t_species']."', sample_s = '".$_POST['sample_s']."', n_ind = '".$_POST['n_ind']."'"
