...
$query = "SELECT st_x(location), st_y(location) FROM themis.positions "
        . "LEFT JOIN themis.navire ON themis.navire.id = themis.positions.id_navire "
        . "WHERE themis.navire.navire = '$navire' "
        . "AND themis.positions.date_p < '$date_f' "
        . "AND themis.positions.date_p > '$date_d' ";

//print $query;

$q_themis = pg_fetch_all(pg_query($query));

foreach($q_themis as $row) {
