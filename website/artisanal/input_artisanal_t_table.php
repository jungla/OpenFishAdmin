...
if ($_POST['submit'] == "Enregistrer") {
  $table = $_POST['table'];
  # find largest ID
  $query = "SELECT max(id) FROM artisanal.t_site_obb";
  ...
  if ($table == 't_site_obb') {
    ...
    if ($lon == '' OR $lat == '') {
      $query = "INSERT INTO artisanal.t_site_obb "
      . "(id, site, strata, region, code, location, active) VALUES ($id+1, '$site', '$strata', '$region', '$code', ST_GeomFromText($point,4326), '$active')";
    } else {
      $query = "INSERT INTO artisanal.t_site_obb "
      . "(id, site, strata, region, code, location, active) VALUES ($id+1, '$site', '$strata', '$region', '$code', '$active')";
    }
