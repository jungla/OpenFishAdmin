...
$_SESSION['filter']['s_owner_id'] = str_replace('\'','',$_POST['s_owner_id']);
...
if ($_GET['s_owner_id'] != "") {$_SESSION['filter']['s_owner_id'] = $_GET['s_owner_id'];}
...
if ($_GET['action'] == 'show') {
    ...
    <form method="post" action="<?php echo $self;?>?source=license&table=owner&action=show" enctype="multipart/form-data">
    <fieldset>
    <table id="no-border"><tr><td><b>Nom du propri&eacute;taire</b></td><td><b>Num&eacute;ro de pi&egrave;ce d'identit&eacute;</b></td><td><b>Nationalit&eacute;</b></td></tr>
    <tr>
    <td>
    <input type="text" size="20" name="s_owner_name" value="<?php echo $_SESSION['filter']['s_owner_name']?>"/>
    </td>
    <td>
    <input type="text" size="20" name="s_owner_id" value="<?php echo $_SESSION['filter']['s_owner_id']?>"/>
    </td>
    <td>
    <select name="f_t_nationality">
        <option value="t_nationality" selected="selected">Tous</option>
        <?php
        $result = pg_query("SELECT id, nationality FROM artisanal.t_nationality ORDER BY t_nationality");
    ...
    if ($_SESSION['filter']['s_owner_name'] != "" OR $_SESSION['filter']['s_owner_id'] != "" OR $_SESSION['filter']['f_t_nationality'] != "") {

        $_SESSION['start'] = 0;

        if ($_SESSION['filter']['s_owner_name'] != "" OR $_SESSION['filter']['s_owner_id'] != "") {
            ...
            $query = "SELECT owner.id, datetime::date, username, first_name, last_name, bday, wives, children, t_card.card, idcard, ycard, address, t_nationality.nationality, telephone, photo_data, comments, "
            . " coalesce(similarity(artisanal.owner.first_name, '".$_SESSION['filter']['s_owner_name']."'),0) + "
            . " coalesce(similarity(artisanal.owner.last_name, '".$_SESSION['filter']['s_owner_name']."'),0) + "
            . " coalesce(similarity(artisanal.owner.idcard, '".$_SESSION['filter']['s_owner_id']."'),0) AS score"
    ...
    while ($results = pg_fetch_row($r_query)) {
        ...
        $query = "SELECT count(infraction.id) FROM artisanal.pirogue "
                ...
                . "WHERE id_owner = '$results[0]'";

        $results_i = 0; #pg_fetch_array(pg_query($query));

        # pirogues
        $query_p = "SELECT count(id) FROM artisanal.pirogue "
        . " WHERE id_owner = '$results[0]'";

        $results_p = pg_fetch_array(pg_query($query_p));

        print "<tr align=\"center\"><td>"
        . "<a href=\"./view_owner.php?id=$results[0]\">Voir</a><br/>";
        if(right_write($_SESSION['username'],2,2)) {
            print "<a href=\"./view_licenses_owner.php?source=$source&table=$table&action=edit&id=$results[0]\">Modifier</a><br/>"
            . "<a href=\"./view_licenses_owner.php?source=$source&table=$table&action=delete&id=$results[0]\" onclick=\"return confirm('Voulez-vous vraiment supprimer l\'enregistrement?')\">Effacer</a>";
        }
        print "</td>";
    ...
    pages($start,$step,$pnum,'./view_licenses_owner.php?source=license&table=owner&action=show&s_owner_name='.$_SESSION['filter']['s_owner_name'].'&s_owner_id='.$_SESSION['filter']['s_owner_id'].'&f_t_nationality='.$_SESSION['filter']['f_t_nationality']);
    ...
    $q_id = "SELECT * FROM artisanal.owner WHERE id = '$id' ORDER BY datetime DESC";
    //print $q_id;
    $r_id = pg_query($q_id);
...
?>
    ...
    <td style="width: 60%">
    <div>
    <img id="cropper" src="<?php print "image.php?id=$id&table=artisanal.owner&photo_data=photo_data";?>" alt="Picture">
    <br/>
    ...
    <script type="text/javascript">
        ...
        cropper.getCroppedCanvas().toBlob(function (blob) {
            ...
            $.ajax('<?php echo $GLOBALS['path']; ?>/artisanal/administration/upload.php?table=owner&id=<?php echo $_GET['id']; ?>', {
                method: "POST",
                data: formData,
                processData: false,
                contentType: false,
    ...
    $id = $_GET['id'];
    $query = "DELETE FROM artisanal.owner WHERE id = '$id'";

    if(!pg_query($query)) {
        msg_queryerror();
//        print $query;
    } else {
        header("Location: ".$_SESSION['http_host']."/artisanal/administration/view_licenses_owner.php?source=$source&table=owner&action=show");
    }
    $controllo = 1;
...
if ($_POST['delete'] == "Supprimer") {
    ...
    $query = "UPDATE artisanal.owner SET "
    . " photo_data = NULL "
    . " WHERE id = '$id'";
...
//        print $query;
        ...
        header("Location: ".$_SESSION['http_host']."/artisanal/administration/view_licenses_owner.php?source=$source&table=$table&action=edit&id=$id");
    }
...
if ($_POST['submit'] == "Enregistrer") {
    ...
    if ($_FILES['photo_file']['tmp_name'] != "") {
        ...
        $photo_data = pg_fetch_result(pg_query("SELECT photo_data FROM artisanal.owner WHERE id = '{".$_POST['id']."}'"), 'photo_data');
    }
    ...
    if ($_POST['new_old']) {
        # check duplicates
        $query = "SELECT id FROM artisanal.owner WHERE last_name='$last_name' AND first_name='$first_name'";
        ...
        if ($nrows > 0) {
            top();
            print "<p>Un propri&eacute;taire avec pr&eacute;nom <b>".$first_name."</b> et nom <b>".$last_name."</b> existe d&eacute;j&egrave;.<br/>";
        ...
        . " wives = '".$wives."', children = '".$children."', t_card = '".$t_card."', idcard = '".$idcard."', ycard = '".$ycard."', address = '".$address."', "
        . " t_nationality = '".$t_nationality."', telephone = '".$telephone."', photo_data = '".$photo_data."', comments = '".$comments."'"
        . " WHERE id = '{".$_POST['id']."}'";
    }
   ...
   $query = str_replace('\'\'', 'NULL', $query);
    ...
    if(!pg_query($query)) {
        ...
        #print $query;
        header("Location: ".$_SESSION['http_host']."/artisanal/administration/view_licenses_owner.php?source=$source&table=owners&action=show");
    }
