...
if ($_GET['action'] == 'show') {
    ...
    if ($_SESSION['filter']['f_duplicate'] == "TRUE") {
        ...
        $_SESSION['start'] = 0;
            ...
            $query = "SELECT similarity(o1.first_name, o2.first_name)*0.3 + similarity(o1.last_name, o2.last_name)*0.7 + "
            . " similarity(o1.first_name, o2.last_name)*0.5 + similarity(o1.last_name, o2.first_name)*0.5 AS sim, "
        ...
        $query = "SELECT owner.id, owner.id, datetime::date, username, first_name, last_name, bday, wives, children, t_card.card, idcard , "
    ...
    while ($results = pg_fetch_row($r_query)) {
        ...
        if ($_SESSION['filter']['f_duplicate'] == 'TRUE') {
          print "<td rowspan=2 ><a href=\"./maintain_licenses_owner.php?f_duplicate=$f_duplicate&source=$source&table=$table&action=merge&id_1=$results[1]&id_2=$results[17]\">Merge</a></td>";
        } else {
          print "<td></td>";
        }
        ...
        print "<td nowrap><a href=\"./view_owner.php?id=$results[1]\">Voir</a></td><td nowrap>$results[2]<br/>$results[3]</td><td>".strtoupper($results[5])."<br/>".ucfirst($results[4])."</td><td nowrap>$results[6]</td><td>$results[7]<br/>$results[8]</td>"
        ...
        if ($_SESSION['filter']['f_duplicate'] == 'TRUE') {
          ...
          print "<td nowrap><a href=\"./view_owner.php?id=$results[17]\">Voir</a></td><td nowrap>$results[18]<br/>$results[19]</td><td>".strtoupper($results[21])."<br/>".ucfirst($results[20])."</td><td nowrap>$results[22]</td><td>$results[23]<br/>$results[24]</td>"
    ...
    pages($start,$step,$pnum,'./maintain_licenses_owner.php?source=autorisation&table=owner&action=show&f_duplicate='.$_SESSION['filter']['f_duplicate']);
    ...
    <form method="post" action="<?php echo $self;?>?source=autorisation&table=owner&action=show&f_duplicate=<?php echo $_SESSION['filter']['f_duplicate'];?>" enctype="multipart/form-data">
    ...
    <table>
      ...
      while ($results = pg_fetch_row($r_query_p)) {
                      print "<tr align=\"center\"><td><a href=\"./view_pirogue.php?id=$results[0]&source=license&table=pirogue\">Voir</a></td><td>$results[2]<br/>$results[1]</td><td>$results[3]</td><td>$results[4]</td><td>$results[5]</td><td>$results[6]</td>";
      }
...
if ($_POST['submit'] == "Enregistrer") {
  ...
  header("Location: ".$_SESSION['http_host']."/artisanal/maintain_licenses_owner.php?source=$source&table=owners&action=show&f_duplicate=$f_duplicate");
