...
if ($_GET['action'] == 'show') {
    ...
    <form method="post" action="<?php echo $self;?>?source=autorisation&table=fisherman&action=show" enctype="multipart/form-data">
    <fieldset>
    <table id="no-border"><tr><td><b>Duplicates</b></td></tr>
    <tr>
    <td>
    <input type="radio" name="f_duplicate" value="TRUE" <?php if($_SESSION['filter']['f_duplicate'] == "TRUE"){ print "checked=\"checked\"";}?> />Oui<br/>
    <input type="radio" name="f_duplicate" value="FALSE" <?php if($_SESSION['filter']['f_duplicate'] == "FALSE" OR $_SESSION['filter']['f_duplicate'] == ""){ print "checked=\"checked\"";}?> />Non<br/>
    </td>
    </tr>
    </table>
    ...
    if ($_SESSION['filter']['f_duplicate'] == "TRUE") {
        ...
        $_SESSION['start'] = 0;
            ...
            $query = "SELECT similarity(o1.first_name, o2.first_name)*0.3 + similarity(o1.last_name, o2.last_name)*0.7 AS sim, "
            ...
            . " FROM artisanal.fisherman o1"
            . " JOIN artisanal.fisherman o2 ON o1.first_name <> o2.first_name AND o1.first_name % o2.first_name"
        ...
        $query = "SELECT fisherman.id, fisherman.id, datetime::date, username, first_name, last_name, bday, wives, children, t_card.card, idcard , "
    ...
    while ($results = pg_fetch_row($r_query)) {
        # infractions
        $query = "SELECT count(infraction.id) FROM infraction.infraction "
                . "WHERE infraction.id_fisherman_1 = '$results[1]' "
                . "OR  infraction.id_fisherman_2 = '$results[1]' "
                . "OR  infraction.id_fisherman_3 = '$results[1]' "
                . "OR  infraction.id_fisherman_4 = '$results[1]' ";

                //print $query;
        $results_i_1 = pg_fetch_array(pg_query($query));

        # cartes
        $query = "SELECT count(id) FROM artisanal.carte "
        . " WHERE id_fisherman = '$results[1]'";
        ...
        if ($_SESSION['filter']['f_duplicate'] == 'TRUE') {
          print "<td rowspan=2 ><a href=\"./maintain_licenses_fisherman.php?f_duplicate=$f_duplicate&source=$source&table=$table&action=merge&id_1=$results[1]&id_2=$results[17]\">Merge</a></td>";
        } else {
          print "<td></td>";
        }
        ...
        print "<td nowrap><a href=\"./view_fisherman.php?id=$results[1]\">Voir</a></td><td nowrap>$results[2]<br/>$results[3]</td><td>".strtoupper($results[5])."<br/>".ucfirst($results[4])."</td><td nowrap>$results[6]</td><td>$results[7]<br/>$results[8]</td>"
        ...
        if ($_SESSION['filter']['f_duplicate'] == 'TRUE') {
          ...
          print "<td nowrap><a href=\"./view_fisherman.php?id=$results[17]\">Voir</a></td><td nowrap>$results[18]<br/>$results[19]</td><td>".strtoupper($results[21])."<br/>".ucfirst($results[20])."</td><td nowrap>$results[22]</td><td>$results[23]<br/>$results[24]</td>"
    ...
    pages($start,$step,$pnum,'./maintain_licenses_fisherman.php?source=autorisation&table=fisherman&action=show&f_duplicate='.$_SESSION['filter']['f_duplicate']);
    ...
    $f_duplicate = $_GET['f_duplicate'];
    $id_1 = $_GET['id_1'];
    $id_2 = $_GET['id_2'];

    //find record info by ID
    $q_id = "SELECT fisherman.id, datetime::date, username, first_name, last_name, bday, wives, children, t_card.card, idcard, "
    . "ycard, address, t_nationality.nationality, telephone, photo_data, comments "
    . "FROM artisanal.fisherman "
    . "LEFT JOIN artisanal.t_card ON artisanal.t_card.id = artisanal.fisherman.t_card "
    . "LEFT JOIN artisanal.t_nationality ON artisanal.t_nationality.id = artisanal.fisherman.t_nationality "
    . "WHERE fisherman.id = '$id_1'";
    ...
    <form method="post" action="<?php echo $self;?>?source=autorisation&table=fisherman&action=show&f_duplicate=<?php echo $_SESSION['filter']['f_duplicate'];?>" enctype="multipart/form-data">
    <h3>D&eacute;tails Pecheur</h3>
    <!--
    MERGE CRITERIA
    ...
    <table>
      ...
      <?php
      $query = "SELECT carte.id, carte, first_name, last_name, license, extract(year FROM carte.date_v), *  FROM artisanal.carte "
              ...
              . "WHERE id_fisherman = '$results_1[0]' OR id_fisherman = '$results_2[0]' "
              . "ORDER BY carte DESC";
      ...
      while ($results = pg_fetch_row($r_query_p)) {
            print "<tr align=\"center\"><td><a href=\"./view_carte.php?id=$results[0]&source=license&table=carte\">Voir</a></td><td>$results[1]</td><td>$results[5]</td>";

            if ($results[13] != "") {
                print "<td><a href=\"./view_license.php?id=$results[12]\">$results[4]</a></td>";
            } else {
                print "<td>Aucun</td>";
      ...
      <?php
      $query = "SELECT infraction.id, infraction.username, id_pv, date_i, t_org.org, name_org, id_pirogue, pir_name, immatriculation, id_owner, owner_first, owner_last, "
            . "owner_idcard, owner_t_card, owner_t_nationality, owner_telephone, id_fisherman_1, fish_first_1, fish_last_1, fish_idcard_1, fish_t_card_1, fish_t_nationality_1, "
            ...
            . "WHERE (id_fisherman_1 = '$results_1[0]' OR id_fisherman_1 = '$results_2[0]' "
            . "OR id_fisherman_2 = '$results_1[0]' OR id_fisherman_2 = '$results_2[0]' "
            . "OR id_fisherman_3 = '$results_1[0]' OR id_fisherman_3 = '$results_2[0]' "
            . "OR id_fisherman_4 = '$results_1[0]' OR id_fisherman_4 = '$results_2[0]') "
        ...
        //print $query;
            ...
            while ($results = pg_fetch_row($r_query_i)) {
                ...
                if ($results[44] !='') {print "[carte: $results[44]]<br/>";}
...
if ($_POST['submit'] == "Enregistrer") {
  ...
  //find record info by ID
  $q_id = "SELECT * FROM artisanal.fisherman WHERE fisherman.id = '$id_1'";
  print $q_id."<br/>";
  $r_id = pg_query($q_id);
  $results_1 = pg_fetch_row($r_id);

  $q_id = "SELECT * FROM artisanal.fisherman WHERE fisherman.id = '$id_2'";
  print $q_id."<br/>";
  $r_id = pg_query($q_id);
  $results_2 = pg_fetch_row($r_id);
  ...
  # DELETE old fishermanS, both view_records_capture
  # INSERT new fisherman with updated records

  $query = "DELETE FROM artisanal.fisherman WHERE id = '$id_1' OR id = '$id_2'";
  pg_query($query);
  //print $query."<br/>";

  $query = "INSERT INTO artisanal.fisherman VALUES (";
  ...
  # UPDATE all records with new records in fisherman, carteS (infractions are linked to carte)
  ...
  # UPDATE cartes and Infractions
  $query = "UPDATE artisanal.carte SET id_fisherman = '$results[0]' WHERE id_fisherman = '$id_1' OR id_fisherman = '$id_2'; ";
  pg_query($query);
  //print $query."<br/>";


  header("Location: ".$_SESSION['http_host']."/artisanal/maintain_licenses_fisherman.php?source=$source&table=fishermans&action=show&f_duplicate=$f_duplicate");
