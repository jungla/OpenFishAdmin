...
if (pg_num_rows($r_query) > 0) {
    ...
    while ($results = pg_fetch_row($r_query)) {
      ...
      print "<td><a href=\"./view_pirogue.php?id=$results[0]\">Voir</a></td><td>$results[3]</td><td>$results[4]</td><td>$results[5]</td><td>$results[6]</td>";
    }

    print "</table><br/>";
}

$query = "SELECT infraction.id, infraction.username, t_infraction.infraction, id_pv, date_i, t_org, name_org, id_pirogue, pir_name, immatriculation, id_owner, owner_first, owner_last, owner_idcard, "
...
if (pg_num_rows($r_query) > 0) {
    ...
    while ($results = pg_fetch_row($r_query)) {
        print "<tr align=\"center\"><td><a href=\"../infractions/view_infraction.php?id=$results[0]\">Voir</a></td>";
        print "<td nowrap>$results[4]</td><td>$results[2]</td>";
        print "<td>".strtoupper($results[12])."<br/>".ucfirst($results[11])."</td>";
        if ($results[17] != '') {
            print "<td><a href=\"./view_fisherman.php?id=$results[17]\">".strtoupper($results[19])."<br/>".ucfirst($results[18])."</a></td>";
        } else {
            print "<td>".strtoupper($results[19])."<br/>".ucfirst($results[18])."</a></td>";
        }
        print "<td>$results[12]</td><td>$results[13]</td><td>$results[14]</td><td>$results[15]</td>"
        . "<td>$results[16]</td><td>$results[17]</td></tr>";
    }
