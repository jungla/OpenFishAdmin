...
if (right_read($username,'2')) {
    ...
    if ($_POST['submit'] == "Enregistrer") {
        ...
        header("Location: ".$_SESSION['http_host']."/artisanal/infractions/artisanal_infractions.php?source=$source&table=infraction&action=show");
...
if (!$controllo) {
    ...
    <select name="t_org">
        <?php
        $result = pg_query("SELECT * FROM infraction.t_org WHERE active ORDER BY t_org");
    ...
    <br/>
    <?php
    $query = "SELECT t_infraction FROM artisanal.t_infraction WHERE id = '$results[0]'";
    ...
    $result = pg_query("SELECT * FROM artisanal.t_infraction ORDER BY infraction");
