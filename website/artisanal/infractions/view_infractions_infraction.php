...
<tr>
    ...
    while ($results = pg_fetch_row($r_query)) {
        ...
        if ($results[7] != '') {
            ...
            print "<td rowspan=$nrows><a href=\"../administration/view_pirogue.php?id=$results[7]\">$pirogue_name[0]<br/>$pirogue_name[1]</a>";
        } else {
        ...
        if ($results[10] != '') {
            ...
            print "<a href=\"../administration/view_owner.php?id=$results[10]\">".strtoupper($owner_name[1])."<br/>".ucfirst($owner_name[0])."</a></td>";
        } else {
        ...
        if ($results[18] != '') {
            ...
            print "<td rowspan=$nrows><a href=\"../administration/view_fisherman.php?id=$results[18]\">".strtoupper($fisherman_name[1])."<br/>".ucfirst($fisherman_name[0])."</a>";
        } else {
        ...
        if ($results[26] != '') {
            ...
            $fisherman_name = pg_fetch_row(pg_query($query));
            print "<a href=\"../administration/view_fisherman.php?id=$results[26]\">".strtoupper($fisherman_name[1])."<br/>".ucfirst($fisherman_name[0])."</a></td>";
        } else {
    ...
    <select name="t_org">
        <?php
        $result = pg_query("SELECT * FROM infraction.t_org ORDER BY org");
        while($row = pg_fetch_row($result)) {
            if ($row[0] == $results[5]) {
              if ($row[2] != 't') {
                print "<option value=\"$row[0]\" selected disabled>".$row[1]."</option>";
              } else {
...
//        print $query;
        ...
        header("Location: ".$_SESSION['http_host']."/artisanal/infractions/view_infractions_infraction.php?source=$source&table=infraction&action=show");
    }
    $controllo = 1;
