...
if (right_write($_SESSION['username'],5,2)) {
  ...
  if ($table == 'maree') {
    ...
    if ($_POST['submit'] == "Enregistrer") {
      ...
      . "(username, datetime, obs_name, t_mission, date_d, time_d, t_site_d, date_r, time_r, t_site_r, n_deb, zone, id_pirogue, immatriculation, "
      . "engine, t_gear_1, length_1, height_1, mesh_min_1, mesh_max_1, t_gear_2, length_2, height_2, mesh_min_2, mesh_max_2, "
      . "t_gear_3, length_3, height_3, mesh_min_3, mesh_max_3, comments, gps_track) "
      . "VALUES ('$username', NOW(), '$obs_name', '$t_mission', '$date', '$time', '$t_site', '$date_r', '$time_r', '$t_site_r', '$n_deb', "
      . "'$zone', '$id_pirogue', '$immatriculation', '$engine', '$t_gear_1', '$mesh_max_1', '$mesh_min_1', '$length_1', '$height_1', "
    ...
    if (!$controllo) {
      ...
      <form method="post" action="<?php echo $self;?>" enctype="multipart/form-data" name="form">
        ...
        <br/>
        <input type="date" size="30" name="date" value="<?php echo $results[4];?>" />
        <br/>
        <br/>
        <b>Heure de d&eacute;part</b>
        <br/>
        <input type="time" size="30" name="heure" value="<?php echo $results[4];?>" />
        <br/>
        <br/>
  ...
  if ($_POST['submit'] == "Enregistrer") {
    ...
    $LA = htmlspecialchars($_POST['LA'],ENT_QUOTES);
    ...
    . "(username, datetime, id_maree, id_action, id_species, t_sex, t_maturity, LT, LA, wgt, t_status, t_action, photo_data, comments) "
    . "VALUES ('$username', NOW(), '$id_maree', '$id_action', '$id_species', '$t_sex', '$t_maturity', '$LT', '$LA', '$wgt', '$t_status', '$t_action', '$photo_data', '$comments');";
  ...
  if (!$controllo) {
    ...
    <form method="post" action="<?php echo $self;?>" enctype="multipart/form-data" name="form">
      ...
      <b>Maree</b>
      <br/>
      <select name="id_maree" id="id_maree" onchange="menu_pop_1('id_maree','id_action','id_maree','wpt','artisanal_catches.obs_action')">
        <?php
        $result = pg_query("SELECT obs_maree.id, concat(obs_maree.immatriculation, pirogue.immatriculation), date_d FROM artisanal_catches.obs_maree LEFT JOIN artisanal.pirogue ON artisanal.pirogue.id = artisanal_catches.obs_maree.id_pirogue ORDER BY date_d");
        while($row = pg_fetch_row($result)) {
          print "<option value=\"$row[0]\">".$row[1]." / ".$row[2]."</option>";
      ...
      <select name="wpt" id="wpt">
        ...
        while($row = pg_fetch_row($result)) {
          print "<option value=\"$row[0]\">WPT ".$row[3].": ".$row[1]." / ".$row[2]."</option>";
        }
        ?>
      </select>
      <br/>
      ...
      <b>LA</b>
      <br/>
      <input type="text" size="10" name="LA" value="<?php echo $results[4];?>" />
      <br/>
      <br/>
      <b>LT</b>
      <br/>
  ...
  if ($_POST['submit'] == "Enregistrer") {
    ...
    $query = "INSERT INTO artisanal_catches.obs_action "
    . "(username, datetime, id_maree, wpt, t_gear, boarded, comments, location) "
    . "VALUES ('$username', NOW(), '$id_maree', '$wpt', '$t_gear', '$boarded', '$comments', $point);";
