...
if ($_GET['action'] == 'show') {
    ...
    <td>
      <select name="f_t_site" class="chosen-select" style="width:100%">
          <option value="enq_maree.t_site" selected="selected">Tous</option>
          <?php
          $result = pg_query("SELECT DISTINCT t_site_obb.id, site FROM artisanal.t_site_obb JOIN artisanal_catches.enq_maree ON artisanal_catches.enq_maree.t_site = artisanal.t_site_obb.id ORDER BY site");
    ...
    <td>
      <select name="f_id_species" class="chosen-select" >
          <option value="enq_catch.id_species" selected="selected">Tous</option>
          <?php
          $result = pg_query("SELECT DISTINCT fishery.species.id, fishery.species.francaise, fishery.species.family, fishery.species.genus, fishery.species.species FROM fishery.species  JOIN artisanal_catches.enq_catch ON fishery.species.id = artisanal_catches.enq_catch.id_species  ORDER BY  fishery.species.family, fishery.species.genus, fishery.species.species");
          while($row = pg_fetch_row($result)) {
    ...
    if ($_SESSION['filter']['f_immatriculation'] != "" OR $_SESSION['filter']['f_pir_name'] != "" OR $_SESSION['filter']['f_id_species'] != "" OR $_SESSION['filter']['f_t_site'] != "") {
        ...
        $query = "SELECT count(DISTINCT enq_maree.id) FROM artisanal_catches.enq_maree "
        . "LEFT JOIN artisanal_catches.enq_catch ON artisanal_catches.enq_maree.id = artisanal_catches.enq_catch.id_maree "
        . "WHERE (enq_maree.t_site = ".$_SESSION['filter']['f_t_site']." OR enq_maree.t_site IS NULL) "
        . "AND (enq_catch.id_species = ".$_SESSION['filter']['f_id_species']." OR enq_catch.id_species IS NULL) ";

        $pnum = pg_fetch_row(pg_query($query))[0];

        if ($_SESSION['filter']['f_immatriculation'] != "" OR $_SESSION['filter']['f_pir_name'] != "") {

            $query = "SELECT DISTINCT enq_maree.id, enq_maree.datetime::date, enq_maree.username, date_d::date, date_r::date, obs_name, t_site_obb.site, id_pirogue, enq_maree.immatriculation, enq_maree.pir_name, enq_maree.t_gear, enq_maree.comments, "
            . " coalesce(similarity(artisanal_catches.enq_maree.immatriculation, '".$_SESSION['filter']['f_immatriculation']."'),0) + "
            . " coalesce(similarity(artisanal.pirogue.immatriculation, '".$_SESSION['filter']['f_immatriculation']."'),0) + "
            . " coalesce(similarity(artisanal_catches.enq_maree.pir_name, '".$_SESSION['filter']['f_pir_name']."'),0) + "
            . " coalesce(similarity(artisanal.pirogue.name, '".$_SESSION['filter']['f_pir_name']."'),0) AS score "
            . "FROM artisanal_catches.enq_maree "
            . "LEFT JOIN artisanal_catches.enq_catch ON artisanal_catches.enq_maree.id = artisanal_catches.enq_catch.id_maree "
            . "LEFT JOIN artisanal.pirogue ON artisanal.pirogue.id = artisanal_catches.enq_maree.id_pirogue "
            . "LEFT JOIN artisanal.t_site_obb ON artisanal.t_site_obb.id = artisanal_catches.enq_maree.t_site "
            . "LEFT JOIN artisanal.t_gear ON artisanal_catches.enq_maree.t_gear = artisanal.t_gear.id "
            . "WHERE (enq_maree.t_site = ".$_SESSION['filter']['f_t_site']." OR enq_maree.t_site IS NULL) "
            . "AND (enq_catch.id_species = ".$_SESSION['filter']['f_id_species']." OR enq_catch.id_species IS NULL) "
            ...
            . "LEFT JOIN artisanal.t_site_obb ON artisanal.t_site_obb.id = artisanal_catches.enq_maree.t_site "
            . "LEFT JOIN artisanal.t_gear ON artisanal_catches.enq_maree.t_gear = artisanal.t_gear.id "
            . "WHERE enq_maree.t_site = ".$_SESSION['filter']['f_t_site']." "
            . "AND enq_catch.id_species = ".$_SESSION['filter']['f_id_species']." "
            . "ORDER BY enq_maree.datetime::date DESC OFFSET $start LIMIT $step";
            }
    } else {
        $query = "SELECT count(enq_maree.id) FROM artisanal_catches.enq_maree";
        $pnum = pg_fetch_row(pg_query($query))[0];
    ...
    while ($results = pg_fetch_row($r_query)) {
        ...
        if(right_write($_SESSION['username'],3,2)) {
            print "<a href=\"./view_catches_enq.php?source=$source&table=$table&action=edit&id=$results[0]\">Modifier</a><br/>"
            . "<a href=\"./view_catches_enq.php?source=$source&table=$table&action=delete&id=$results[0]\" onclick=\"return confirm('Voulez-vous vraiment supprimer l\'enregistrement?')\">Effacer</a>";
        }
        print "</td>";
        ...
        . "FROM artisanal_catches.enq_catch "
        . "LEFT JOIN fishery.species ON artisanal_catches.enq_catch.id_species = fishery.species.id "
    ...
    pages($start,$step,$pnum,'./view_catches_enq.php?source=artisanal&table=captures&action=show&f_immatriculation='.$_SESSION['filter']['f_immatriculation'].'&f_id_species='.$_SESSION['filter']['f_id_species'].'&f_t_site='.$_SESSION['filter']['f_t_site'].'&f_t_study='.$_SESSION['filter']['f_t_study']);
    ...
    $q_id = "SELECT * FROM artisanal_catches.enq_maree WHERE id = '$id' ";
    //print $q_id;
    $r_id = pg_query($q_id);
...
<br/>
<?php
$query = "SELECT * FROM artisanal_catches.enq_catch WHERE id_maree = '$results[0]'";
...
<input type="submit" value="Enregistrer" name="submit"/>
    ...
    $query = "DELETE FROM artisanal_catches.enq_maree WHERE id = '$id'";
    if(!pg_query($query)) {
        msg_queryerror();
    }

    $query = "DELETE FROM artisanal_catches.enq_catch WHERE id_maree = '$id'";
    if(!pg_query($query)) {
        msg_queryerror();
//        print $query;
    }

    header("Location: ".$_SESSION['http_host']."/artisanal/catches/view_catches_enq.php?source=$source&table=$table&action=show");
...
if ($_POST['submit'] == "Enregistrer") {
            ...
            $query = "UPDATE artisanal_catches.enq_maree SET "
            ...
            for($i = 0; $i < sizeof($_POST['id_species']); $i++) {
              ...
              $query = "INSERT INTO artisanal_catches.enq_catch "
