...
if ($_GET['action'] == 'show') {
        ...
        <tr>
          <td>
            <select name="f_id_maree" class="chosen-select" style="width:100%">
              <option value="obs_sharks.id_maree" selected="selected">Tous</option>
              <?php
              $result = pg_query("SELECT DISTINCT obs_sharks.id_maree, date_d, concat(pirogue.immatriculation, obs_maree.immatriculation) FROM artisanal_catches.obs_sharks "
  ...
  <table id="small">
    ...
    if ($_SESSION['filter']['f_id_maree'] != "" OR $_SESSION['filter']['f_id_species'] != "" OR $_SESSION['filter']['f_observateur'] != "") {
      ...
      $query = "SELECT count(DISTINCT obs_sharks.id) FROM artisanal_catches.obs_sharks "
      . "WHERE obs_sharks.id_maree = ".$_SESSION['filter']['f_id_maree']." ";
      ...
      if ($_SESSION['filter']['f_immatriculation'] != "" OR $_SESSION['filter']['f_observateur'] != "") {
        ...
        . " t_sex.sex, t_maturity.maturity, LT, LD, wgt, t_status.status, t_action.action, photo_data, obs_sharks.comments,  "
      ...
      $query = "SELECT count(obs_sharks.id) FROM artisanal_catches.obs_sharks";
      $pnum = pg_fetch_row(pg_query($query))[0];
      ...
      . "LEFT JOIN artisanal_catches.t_status ON artisanal_catches.t_status.id = artisanal_catches.obs_sharks.t_status "
      . "LEFT JOIN artisanal_catches.t_action ON artisanal_catches.t_action.id = artisanal_catches.obs_sharks.t_action "
      . "ORDER BY obs_sharks.datetime::date DESC OFFSET $start LIMIT $step";
    }
    ...
    while ($results = pg_fetch_row($r_query)) {
      ...
      . "<td>$results[18]</br>$results[19]</td><td>LD: $results[20]<br/>LT: $results[21]</td>";
    ...
    pages($start,$step,$pnum,'./view_catches_obs_sharks.php?source=obs_catches&table=actions&action=show&f_id_maree='.$_SESSION['filter']['f_id_maree'].'&f_observateur='.$_SESSION['filter']['f_observateur']);
    ...
    $q_id = "SELECT *, wpt FROM artisanal_catches.obs_sharks LEFT JOIN artisanal_catches.obs_action ON artisanal_catches.obs_action.id = artisanal_catches.obs_sharks.id_action  WHERE obs_sharks.id = '$id'";
    //print $q_id;
    $r_id = pg_query($q_id);
...
?>
    ...
    <td style="width: 60%">
    <div>
    <img id="cropper" src="<?php print "image.php?id=$id&table=artisanal_catches.obs_sharks&photo_data=photo_data";?>" alt="Picture">
    <br/>
    ...
    <script type="text/javascript">
        ...
        cropper.getCroppedCanvas().toBlob(function (blob) {
            ...
            $.ajax('<?php echo $GLOBALS['path']; ?>/artisanal/catches/upload.php?table=artisanal_catches.obs_sharks&id=<?php echo $_GET['id']; ?>', {
                method: "POST",
                data: formData,
                processData: false,
                contentType: false,
    ...
    <form method="post" action="<?php echo $self;?>" enctype="multipart/form-data" name="form">
      ...
      <select name="wpt" id="wpt">
        ...
        <?php
        $result = pg_query("SELECT wpt FROM artisanal_catches.obs_sharks LEFT JOIN artisanal_catches.obs_action ON artisanal_catches.obs_action.id = artisanal_catches.obs_sharks.id_action WHERE obs_sharks.id = '$results[0]' AND obs_sharks.id_maree = '$results[3]' ORDER BY wpt");
        while($row = pg_fetch_row($result)) {
          if ($row[0] == $results[21]) {
      ...
      <b>Poids</b> [kg]
      <br/>
      <input type="text" size="10" name="wgt" value="<?php echo $results[10];?>" />
      <br/>
      <br/>
      <b>LD</b> [cm]
      <br/>
      <input type="text" size="10" name="LD" value="<?php echo $results[8];?>" />
      <br/>
      <br/>
      <b>LT</b> [cm]
      <br/>
      <input type="text" size="10" name="LT" value="<?php echo $results[9];?>" />
      <br/>
      <br/>
      <b>Status</b>
      <br/>
      ...
      <b>Commentaires</b>
      <br/>
      <textarea cols=30 rows=3 name="comments"><?php echo $results[14];?></textarea>
      <br/>
      <br/>
  ...
  $query = "DELETE FROM artisanal_catches.obs_sharks WHERE id = '$id'";
  if(!pg_query($query)) {
    msg_queryerror();
  }

  header("Location: ".$_SESSION['http_host']."/artisanal/catches/view_catches_obs_sharks.php?source=$source&table=$table&action=show");
...
if ($_POST['delete'] == "Supprimer") {
    ...
    $query = "UPDATE artisanal_catches.sharks SET "
    . " photo_data = NULL "
    . " WHERE id = '$id'";
...
//        print $query;
        ...
        header("Location: ".$_SESSION['http_host']."/artisanal/catches/view_catches_obs_sharks.php?source=$source&table=$table&action=edit&id=$id");
    }
...
if ($_POST['submit'] == "Enregistrer") {
    ...
    if ($_FILES['photo_file']['tmp_name'] != "") {
        ...
        $photo_data = pg_fetch_result(pg_query("SELECT photo_data FROM artisanal_catches.obs_sharks WHERE id = '{".$_POST['id']."}'"), 'photo_data');
    }

  $id = $_POST['id'];
  # id, datetime, username, id_maree, id_action, id_species, t_sex, t_maturity, LT, LD, wgt, t_status, t_action, photo_data, comments
  ...
  $query = "UPDATE artisanal_catches.obs_sharks SET "
  ...
  . "LT = '$LT', LD = '$LD', wgt = '$wgt', t_status = '$t_status', t_action = '$t_action', photo_data = '$photo_data', comments = '$comments' "
  . "WHERE id = '$id'";
