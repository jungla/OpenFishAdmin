...
if ($_GET['action'] == 'show') {
        ...
        <tr>
          <td>
            <select name="f_id_maree" class="chosen-select" style="width:100%">
              <option value="obs_turtles.id_maree" selected="selected">Tous</option>
              <?php
              $result = pg_query("SELECT DISTINCT obs_turtles.id_maree, date_d, concat(pirogue.immatriculation, obs_maree.immatriculation) FROM artisanal_catches.obs_turtles "
  ...
  <table id="small">
    ...
    if ($_SESSION['filter']['f_id_maree'] != "" OR $_SESSION['filter']['f_id_species'] != "" OR $_SESSION['filter']['f_observateur'] != "") {
      ...
      //id, username, datetime, id_maree, id_action, id_species, t_sex, t_maturity, bague, integrity, fibrop, epibionte, LT, wgt, t_status, t_action, photo_data, comments

      $_SESSION['start'] = 0;

      $query = "SELECT count(DISTINCT obs_turtles.id) FROM artisanal_catches.obs_turtles "
      . "WHERE obs_turtles.id_maree = ".$_SESSION['filter']['f_id_maree']." ";
      ...
      $query = "SELECT count(obs_turtles.id) FROM artisanal_catches.obs_turtles";
      $pnum = pg_fetch_row(pg_query($query))[0];
      ...
      . "LEFT JOIN artisanal_catches.t_status ON artisanal_catches.t_status.id = artisanal_catches.obs_turtles.t_status "
      . "LEFT JOIN artisanal_catches.t_action ON artisanal_catches.t_action.id = artisanal_catches.obs_turtles.t_action "
      . "ORDER BY obs_turtles.datetime::date DESC OFFSET $start LIMIT $step";
    }
    ...
    pages($start,$step,$pnum,'./view_catches_obs_turtles.php?source=obs_catches&table=actions&action=show&f_id_maree='.$_SESSION['filter']['f_id_maree'].'&f_observateur='.$_SESSION['filter']['f_observateur']);
    ...
    $q_id = "SELECT *, wpt FROM artisanal_catches.obs_turtles LEFT JOIN artisanal_catches.obs_action ON artisanal_catches.obs_action.id = artisanal_catches.obs_turtles.id_action  WHERE obs_turtles.id = '$id'";
    //print $q_id;
    $r_id = pg_query($q_id);
...
?>
    ...
    <td style="width: 60%">
    <div>
    <img id="cropper" src="<?php print "image.php?id=$id&table=artisanal_catches.obs_turtles&photo_data=photo_data";?>" alt="Picture">
    <br/>
    ...
    <script type="text/javascript">
        ...
        cropper.getCroppedCanvas().toBlob(function (blob) {
            ...
            $.ajax('<?php echo $GLOBALS['path']; ?>/artisanal/catches/upload.php?table=artisanal_catches.obs_turtles&id=<?php echo $_GET['id']; ?>', {
                method: "POST",
                data: formData,
                processData: false,
                contentType: false,
    ...
    <form method="post" action="<?php echo $self;?>" enctype="multipart/form-data" name="form">
      ...
      <select name="wpt" id="wpt">
        ...
        <?php
        $result = pg_query("SELECT wpt FROM artisanal_catches.obs_turtles LEFT JOIN artisanal_catches.obs_action ON artisanal_catches.obs_action.id = artisanal_catches.obs_turtles.id_action WHERE obs_turtles.id = '$results[0]' AND obs_turtles.id_maree = '$results[3]' ORDER BY wpt");
        while($row = pg_fetch_row($result)) {
          if ($row[0] == $results[21]) {
      ...
      <b>Code bague</b>
      <br/>
      <input type="text" size="30" name="bague" value="<?php echo $results[8];?>" />
      <br/>
      <br/>
      <b>Description integrite</b>
      <br/>
      <input type="text" size="30" name="integrity" value="<?php echo $results[9];?>" />
      <br/>
      <br/>
      <b>Presence fibropapillomatosis</b>
      <br/>
      <input type="text" size="30" name="fibrop" value="<?php echo $results[10];?>" />
      <br/>
      <br/>
      <b>Presence epibiontes</b>
      <br/>
      <input type="text" size="30" name="epibionte" value="<?php echo $results[11];?>" />
      <br/>
      <br/>
      <b>LT</b> [cm]
      <br/>
      <input type="text" size="10" name="LT" value="<?php echo $results[12];?>" />
      <br/>
      <br/>
      <b>Poids</b> [kg]
      <br/>
      <input type="text" size="10" name="wgt" value="<?php echo $results[13];?>" />
      <br/>
      <br/>
      <b>Status</b>
      <br/>
      ...
      <b>Commentaires</b>
      <br/>
      <textarea cols=30 rows=3 name="comments"><?php echo $results[17];?></textarea>
      <br/>
      <br/>
  ...
  $query = "DELETE FROM artisanal_catches.obs_turtles WHERE id = '$id'";
  if(!pg_query($query)) {
    msg_queryerror();
  }

  header("Location: ".$_SESSION['http_host']."/artisanal/catches/view_catches_obs_turtles.php?source=$source&table=$table&action=show");
...
if ($_POST['delete'] == "Supprimer") {
    ...
    $query = "UPDATE artisanal_catches.turtles SET "
    . " photo_data = NULL "
    . " WHERE id = '$id'";
...
//        print $query;
        ...
        header("Location: ".$_SESSION['http_host']."/artisanal/catches/view_catches_obs_turtles.php?source=$source&table=$table&action=edit&id=$id");
    }
...
if ($_POST['submit'] == "Enregistrer") {
    ...
    if ($_FILES['photo_file']['tmp_name'] != "") {
        ...
        $photo_data = pg_fetch_result(pg_query("SELECT photo_data FROM artisanal_catches.obs_turtles WHERE id = '{".$_POST['id']."}'"), 'photo_data');
    }
  ...
  $query = "UPDATE artisanal_catches.obs_turtles SET "
  ...
  . "bague = '$bague', integrity = '$integrity', fibrop = '$fibrop', epibionte = '$epibionte', LT = '$LT', wgt = '$wgt', t_status = '$t_status', t_action = '$t_action', photo_data = '$photo_data', comments = '$comments' "
  . "WHERE id = '$id'";
