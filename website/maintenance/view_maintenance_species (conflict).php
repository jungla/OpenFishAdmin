...
if ($_GET['action'] == 'show') {
        ...
        <tr>
          ...
          <td>
            <select name="f_category">
              <option value="category">Tous</option>
              <?php
  ...
  <table id="small">
    ...
    if ($_SESSION['filter']['f_family'] != "" OR $_SESSION['filter']['f_genus'] != "" OR $_SESSION['filter']['f_category'] != "" OR $_SESSION['filter']['f_francaise'] != "") {

      $_SESSION['start'] = 0;

      $query = "SELECT count(*) FROM fishery.species "
      . "WHERE species.family=".$_SESSION['filter']['f_family']." "
      . "AND species.genus = ".$_SESSION['filter']['f_genus']. " "
      . "AND species.category LIKE '%".$_SESSION['filter']['f_category']."%'";

      $pnum = pg_fetch_row(pg_query($query))[0];

      if ($_SESSION['filter']['f_francaise'] != "") {
        $query = "SELECT *, "
        . " coalesce(similarity(artisanal_catches.obs_maree.immatriculation, '".$_SESSION['filter']['f_francaise']."'),0) as score "
        . "FROM fishery.species "
        . "WHERE species.family=".$_SESSION['filter']['f_family']." "
        . "AND species.genus = ".$_SESSION['filter']['f_genus']." "
        . "AND species.category LIKE '%".$_SESSION['filter']['f_category']."%'"
        . "ORDER BY score, family, genus, species OFFSET $start LIMIT $step";
    ...
    print "</table>";
    pages($start,$step,$pnum,'./view_maintenance_species.php?source=species&action=show');
