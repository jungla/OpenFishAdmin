...
if(logged($username,$password)) {
	...
	$radice = $_SERVER['HTTP_HOST'];
        ...
        <ul>
        <li>Nom d'utilisateur: <b>$username</b></li>
        <li>Adresse email: <b>$r_query[2]</b></li>
        <li>Projets:</li>
            <ol>";
            for($i = 0; $i < 5; $i++) {
                if ($r_query[$i+3] == 't') {
    ...
    if ($_POST['submit'] == 'Connexion') {
        ...
        if (trim($username) == "" OR trim($password) == "") {
            echo "<strong><br/>Veuillez ins&eacute;rer le nom d'utilisateur et le mot de passe.</strong><br/><br/><input type=\"button\" value=\"Connexion\" onclick=\"history.back();\">";
            $controllo = 1;
        } else {
