...
pdf.cell(100, 14, 'genere a partir de la base de donnees data.gabonbleu.org',border=0,ln=1,align='l')
pdf.set_font('Raleway-Bold', '', 18)
pdf.cell(100, 14, 'Liste des infractions enregistrees en '+month_l[month-1]+' '+str(year),border=0,ln=1,align='l')
...
for infraction in infractions:
 ...
 for t_infraction in t_infractions:
  pdf.ln(lh)
  pdf.cell(50,lh,chr(127)+' '+t_infraction[0],border=0) 
 ...
 if (infraction[10] is not None) :
  ...
  cur.execute(query)
  owner_name = np.asarray(cur.fetchall())
  if len(owner_name) != 0:
   print infraction[2], infraction[3]
...
# #map = Basemap(projection='merc',llcrnrlat=lat_0,urcrnrlat=lat_1,llcrnrlon=lon_0,urcrnrlon=lon_1,resolution='f')
# 
# parks_shp = './shp/Aires_Protegees_update20170609_PN_May_EPSG4326'
# amps_shp = './shp/AiresProtegeesAquatiques_20170601_Final_EPSG4326_2D'
# 
# fig = plt.figure()
# ax = fig.add_subplot(111)
# map.drawcoastlines(linewidth=0.5)
...
#  plt.text(x[i],y[i],labels[i],zorder=100,size=6)
 ...
 #plt.savefig('./map_infractions_latest.png',dpi=300,bbox_inches='tight')
 #print       './map_infractions_latest.png'
 #plt.close()
...
pdf.output('/var/www/script_python/GB_report_infractions.pdf', 'F')
