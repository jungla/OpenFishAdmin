...
#  lat_t.append(new_p[3])
  ...
  query = "SELECT * FROM artisanal.trackers_pelagic WHERE name = '"+new_p[1]+"' ORDER BY date_t DESC LIMIT 1"
  ...
  if len(old_p) == 0 or old_p[0][2] < datetime.datetime.strptime(new_p[0],'%Y-%m-%d %H:%M:%S'):
   query = "INSERT INTO artisanal.trackers_pelagic (date_t, name, speed, range, heading, location) VALUES (%s, %s, %s, %s, %s, ST_GeomFromText('POINT(%s %s)',4326))"
